...
namespace Tree
{
    ...
    public class Tree : ITree
    {
        ...
        /// <summary>
        /// метод ищет значение F в дереве методом поиска в глубину
        /// возвращает значение элемента, если он найден
        /// </summary>
        /// <param name="F"></param>
        /// <returns></returns>
        public TreeNode DFS(int F)
        {
            Console.WriteLine($"Ищем элемент {F}");

            Stack<TreeNode> sf = new Stack<TreeNode>();
            sf.Push(_head);
            int j = 0; 
            while (sf != null)
            {
                TreeNode testEl = sf.Pop(); // извлекаем из стека элемент для проверки

                Console.WriteLine($"Проверяем {j+1} элемент = {testEl.Value}");
                ...
                if (testEl.LeftChild != null)
                    sf.Push(testEl.LeftChild);
                if (testEl.RightChild != null)
                    sf.Push(testEl.RightChild);
                j++;
            }
        ...
        /// <summary>
        /// метод ищет значение F в дереве методом поиска в ширину
        /// </summary>
        /// <param name="F"></param>
        public TreeNode BFS(int F)
        {
            Stack<int> numbers = new Stack<int>();
            Console.WriteLine($"Ищем элемент {F}");

            Queue<TreeNode> sf = new Queue<TreeNode>();
            sf.Enqueue(_head);
            int j = 0;
            while (sf != null)
            {
                TreeNode testEl = sf.Dequeue(); // извлекаем из стека элемент для проверки
                ...
                if (testEl.LeftChild != null)
                    sf.Enqueue(testEl.LeftChild);
                if (testEl.RightChild != null)
                    sf.Enqueue(testEl.RightChild);
                j++;
            }
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine();
            Console.WriteLine("Проводим поиск методом DEPTH FIRST SEARCH");
            Console.WriteLine();
            tree.DFS(71);
            Console.WriteLine();
            Console.WriteLine("Проводим поиск методом BREADTH FIRST SEARCH");
            tree.BFS(71);
            Console.WriteLine();
            Console.WriteLine("Проводим поиск методом DEPTH FIRST SEARCH");
            Console.WriteLine();
